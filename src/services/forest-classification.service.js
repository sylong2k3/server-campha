...
const dbgTime = (tag, msg, t0) => {
    if (DEBUG) {
        console.debug(`[FOREST-CLS:DBG:${tag}] ${msg} (${Date.now() - t0}ms)`);
    }
};
...
const buildSnapshotComparison = async (snapshot, districtAreas) => {
    if (!snapshot || !['completed', 'published'].includes(snapshot.status)) {
        return null;
    }

    const previousSnapshot = await repo.getPreviousCompleted(snapshot.year, snapshot.month);
    if (!previousSnapshot?.province_summary) {
        return null;
    }
    ...
    const districts =
        previousDistrictAreas.length > 0
...
const getLatest = async () => {
    ...
    if (!snapshot) {
        if (newest) {
            dbgTime(
                'GET_LATEST',
                `pending id=${newest.id} status=${newest.status} y/m=${newest.year}/${newest.month}`,
                t0,
            );
            return {
                snapshot: newest,
...
const queryForPeriod = async (year, month, userId = null) => {
    ...
    runAnalysis(year, month, { trigger: 'user', requestedBy: userId }).catch((err) =>
        console.error(`[FOREST] user query y=${year} m=${month}:`, err.message),
    );
    ...
    if (!pending) {
        pending = { id: null, year, month, status: 'computing' };
    }
