...
async function runForestDistrictRasterExport({
    ...
    const persistSummary = () =>
        repo.updateDistrictExportSummary(snapshot.id, {
    ...
    for (const district of districtGeoJson) {
        ...
        try {
            const districtGeom =
                district.epsg && district.epsg !== 4326
            ...
            const url = await new Promise((resolve, reject) => {
                const timer = setTimeout(
                    () =>
                        reject(
                            new Error(`getDownloadURL timeout huyện=${code} sau ${timeoutMs}ms`),
                        ),
                    timeoutMs,
                );
                classifiedForDownload
