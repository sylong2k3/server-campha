...
const _completeLogin = async (user, context = {}, metadata = {}) => {
    ...
    await Promise.all([
        ...
        _logActivity(user.id, 'login', 'success', context, metadata),
    ]);
    return {
        user: _sanitizeUser(user, context.lang),
        accessToken: tokens.accessToken,
...
const refresh = async (refreshToken, context = {}) => {
    ...
    if (!storedToken) {
        if (decoded?.userId) {
            await Promise.all([
                tokenRepository.deleteAllUserTokens(decoded.userId),
                userRepository.incrementTokenVersion(decoded.userId),
            ]);
...
const resetPassword = async ({ token, newPassword }, context = {}) => {
    ...
    await Promise.all([
        ...
        userRepository.incrementTokenVersion(user.id),
        tokenRepository.markPasswordResetTokenUsed(resetToken.id),
        tokenRepository.invalidateUserResetTokens(user.id),
        tokenRepository.deleteAllUserTokens(user.id),
    ]);
...
const updateMe = async (userId, data, file, context = {}) => {
    ...
    const normalized = {
        ...
        avatarUrl: avatarUrl !== undefined ? _normalizeNullable(avatarUrl) : undefined,
    };
...
const _logActivity = (userId, action, status, context = {}, metadata = {}) =>
    activityLogger.logActivity('[AUTH]', {
        userId,
        action,
        status,
...
const invalidateSessions = (userId) => userRepository.incrementTokenVersion(userId);
