...
const dbg = (tag, msg) => {
    if (DEBUG) {
        console.debug(`[FOREST-CLS:DBG:${tag}] ${msg}`);
    }
};
...
async function sendTop3ChangesAlert(snapshot, prevSnapshot, provinceSummary) {
    try {
        ...
        for (let i = 0; i < cfg.CLASS_NAMES.length; i++) {
            ...
            const pct = prevHa === 0 ? 100 : ((currHa - prevHa) / prevHa) * 100;
            changes.push({
                classId: i,
        ...
        if (top3[0].absPct < threshold) {
            dbg(
                'ALERT',
                `skip — top-1 change ${top3[0].absPct.toFixed(2)}% < threshold ${threshold}%`,
            );
            return;
        }
        ...
        const lines = top3.map((c, idx) => {
            const sign = c.pct >= 0 ? '+' : '';
            return (
                `  ${idx + 1}. ${c.name}: ${sign}${c.pct.toFixed(1)}% ` +
...
const buildAreaMetric = (currentHa, previousHa) => {
    ...
    const changePct =
        previous > 0 ? roundComparisonValue((deltaHa / previous) * 100) : current === 0 ? 0 : null;
    return { currentHa: current, previousHa: previous, deltaHa, changePct };
};

const sumForestByClass = (byClass = {}) =>
    cfg.FOREST_CLASS_IDS.reduce((sum, classId) => sum + (Number(byClass[classId]) || 0), 0);

const sumAllByClass = (byClass = {}) =>
    Object.values(byClass).reduce((sum, areaHa) => sum + (Number(areaHa) || 0), 0);
