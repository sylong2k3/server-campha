...
/**
 * Shared 13-class Forest Classification pipeline — preserved from scientific
 * model v5.3 while the operational AOI is supplied separately.
 *
 * So với port v3 cũ:
...
function prepL57(img) {
    return img
        .select(['SR_B1', 'SR_B2', 'SR_B3', 'SR_B4', 'SR_B5', 'SR_B7'], BANDS)
        .multiply(0.0000275)
        .add(-0.2)
...
function maskS2(img) {
    ...
    return img
        .select(['B2', 'B3', 'B4', 'B8A', 'B11', 'B12'], BANDS)
        .multiply(0.0001)
        .clamp(0, 1)
        .toFloat()
        .updateMask(
...
function bsi(img, name) {
    return img
        .expression('((S+R)-(N+B))/((S+R)+(N+B)+1e-6)', {
            S: img.select('swir1'),
            R: img.select('red'),
            N: img.select('nir'),
            B: img.select('blue'),
...
function buildFeatureImage(year, month, region) {
    ...
    const masterStart = ee.Date(
        ee.Algorithms.If(g.start.difference(baseStart, 'day').lt(0), g.start, baseStart),
    );
...
function buildThresholds(im, hasTexture, dwBuilt) {
    ...
    if (dwBuilt) {
        T.other = T.other.or(dwBuilt.gte(cfg.DW_BUILT_MIN).and(gN.lte(0.68)));
    }
    ...
    const evClosed = gN
        ...
        .and(bsiB.lte(-0.08))
        .and(elev.gte(200));
    const evSecondary = gN
        .gte(0.62)
        .and(gN.lt(0.72))
        .and(fN.gte(0.56))
        .and(dN.gte(0.56))
        .and(amp.lte(0.16))
        .and(recov.abs().lte(0.08))
        .and(gM.gte(0.18))
        .and(gM.lte(0.32))
        .and(gE.gte(0.26))
        .and(gB.gte(0.36))
...
function buildRelaxedLabel(im, naturalCore, dwBuilt) {
    ...
    if (dwBuilt) {
        c1.push({ c: dwBuilt.gte(0.06), w: 3 });
    }
    S[1] = score(c1).multiply(notCore);
    S[2] = score([
        { c: btw(gN, 0.46, 0.92), w: 1 },
        { c: dN.gte(0.36), w: 1 },
        { c: gM.lte(0.29), w: 3 },
        { c: btw(elev, 300, 1300), w: 1 },
        { c: slope.lte(24), w: 2 },
        { c: recov.gte(0.04).or(amp.lte(0.24)), w: 2 },
    ]).multiply(notCore);
    S[3] = score([
        { c: amp.gte(0.1), w: 2 },
        { c: dN.lte(0.6), w: 2 },
    ...
    S[4] = score([
        { c: elev.gte(880), w: 2 },
        { c: btw(gN, 0.56, 0.88), w: 1 },
        { c: amp.lte(0.18), w: 1 },
        { c: btw(gM, 0.15, 0.35), w: 1 },
        { c: btw(gS, 0.054, 0.118), w: 2 },
    ]);
    S[5] = score([
        { c: btw(gN, 0.6, 0.95), w: 1 },
        { c: fN.gte(0.54), w: 2 },
        { c: dN.gte(0.54), w: 2 },
        { c: amp.lte(0.17), w: 1 },
        { c: gM.gte(0.17), w: 2 },
    ...
    S[7] = score([
        { c: fN.lte(0.48), w: 2 },
        { c: drop.gte(0.24), w: 2 },
        { c: recov.lte(0.12), w: 1 },
        { c: elev.lte(900), w: 1 },
        { c: gN.gte(0.58), w: 1 },
    ]);
    S[8] = score([
        { c: btw(gN, 0.58, 0.9), w: 1 },
        { c: btw(amp, 0.07, 0.3), w: 2 },
        { c: btw(gM, 0.09, 0.33), w: 2 },
        { c: gB.gte(0.32), w: 2 },
        { c: btw(gS, 0.046, 0.08), w: 2 },
        { c: btw(elev, 250, 1450), w: 1 },
        { c: slope.gte(4), w: 1 },
    ]);
    S[9] = score([
        { c: gS.gte(0.076), w: 3 },
        { c: btw(gN, 0.48, 0.84), w: 1 },
        { c: amp.lte(0.18), w: 1 },
        { c: slope.lte(13), w: 2 },
        { c: gM.lte(0.26), w: 2 },
        { c: gE.lte(0.36), w: 2 },
...
function buildLabel(im, T, dwBuilt, region) {
    ...
    const order = [
        ...
        { v: 3, m: c3 },
        { v: 2, m: c2 },
        { v: 9, m: T.plantation.and(keep) },
        { v: 6, m: T.conifer.and(keep) },
        { v: 5, m: T.evergreen.and(keep) },
        { v: 8, m: T.bamboo.and(keep) },
        { v: 4, m: T.mixed.and(keep) },
        { v: 7, m: T.deciduous.and(keep) },
        { v: 10, m: c10 },
    ];
    const hard = ee
        .ImageCollection(order.map(({ v, m }) => ee.Image.constant(v).toInt16().updateMask(m)))
        .mosaic()
        .rename('class')
        .toInt16();
    return {
...
function buildPriorityLabel(entries, region) {
    ...
    return ee.ImageCollection(layers).mosaic().rename('class').toInt16().clip(region);
}
...
function otherLandMask(region, assetId = cfg.OTHER_LAND_ASSET) {
    ...
    return ee
        .Image()
        .byte()
        .paint(ee.FeatureCollection(assetId), 1)
        .unmask(0)
        .gt(0)
        .rename('other_land')
        .clip(region);
...
function computeQuotas(budget) {
    ...
    return roots.map((r, classId) =>
        cfg.NO_TRAIN_CLASSES.includes(classId)
            ? 0
            : Math.max(cfg.SAMPLE_MIN, Math.min(cfg.SAMPLE_MAX, Math.round((budget * r) / sum))),
    );
}
...
function buildThresholdLabel(base, dryIdx, wetIdx, dem, region) {
    ...
    console.warn(
        '[FOREST-CLS] buildThresholdLabel legacy shim — pipeline v4.1 đã tự dựng threshold trong runRfClassification',
    );
    void base;
    void dryIdx;
    void wetIdx;
...
function buildDatasetLabel(featureImage, thresholdLabel, year, region) {
    ...
    console.warn(
        '[FOREST-CLS] buildDatasetLabel legacy shim — pipeline v4.1 đã bỏ DW/WorldCover epoch-lệch',
    );
    void featureImage;
    void year;
    void region;
...
async function runRfClassification(year, region, regionGeom, opts = {}) {
    ...
    const log =
        logger ||
        makeStageLogger(liteMode ? 'FOREST-CLS-RF-LITE' : 'FOREST-CLS-RF', {
    ...
    for (let i = 0; i < cfg.AREA_PRIOR.length; i++) {
        if (
            !cfg.NO_TRAIN_CLASSES.includes(i) &&
            cfg.AREA_PRIOR[i] > 0 &&
            cfg.AREA_PRIOR[i] < 20000
        ) {
            rareIdx.push(i);
        }
    }
    ...
    log.mark(
        liteMode ? 'RF quotas (LITE)' : 'RF quotas',
        ...
        await log.run(
            ...
            async () => {
                ...
                if (groundTruthGeoJson) {
                    ...
                    gtFC = ee
                        .FeatureCollection(groundTruthAssetId)
    ...
    const thrQuota = hasGT
        ? quotasPerClass.map((n) => Math.round(n * 0.5))
        : quotasPerClass.slice();
            ...
            () =>
                Promise.resolve(
                    sampleLabel(
                        ...
                        rarePoints,
                        regionGeom,
                        cfg.SAMPLE_SCALE_RARE_M,
                        clampSeed(rfSeed * 100 + 7),
                        excl,
                        'threshold_rare',
                    ),
                ),
    ...
    if (hasGT) {
        ...
        inputTestSamples = await log.run('Sample GT-test (spatially split holdout) [LAZY]', () =>
            Promise.resolve(
                sampleGT(
                    image,
                    gtTestFC,
    ...
    if (shouldComputeOob || !skipStats) {
        ...
        if (sampleHist) {
            ...
            for (let k = 0; k < cfg.CLASS_NAMES.length; k++) {
                if (cfg.NO_TRAIN_CLASSES.includes(k)) {
                    continue;
                }
                const n = Number(sampleHist[String(k)] || 0);
                if (n >= cfg.MIN_TRAIN_PER_CLASS) {
                    present.push(k);
                }
            }
            log.mark(
    ...
    if (!rfSkipReason) {
        ...
        classified = await log.run('Classify + JRC permanent-water override [LAZY]', () =>
            Promise.resolve(
                image
                    .classify(classifier)
                    ...
                    .where(F.dataMask.not(), cfg.NODATA_ID)
                    .clip(region),
            ),
        );
        ...
        if (shouldComputeOob) {
            oobPct = await log.run(
                ...
                async () => {
                    const diagnostics = ee
                        .Dictionary(classifier.explain())
                    ...
                    if (!Number.isFinite(oobError)) {
                        throw new Error(
                            'GEE classifier diagnostics did not return outOfBagErrorEstimate',
                        );
                    }
                    const pct = Math.max(0, Math.min(100, (1 - oobError) * 100));
                    if (oobError < 0.05) {
                        log.mark(
                            '⚠ OOB > 95% khi không có GT',
            ...
            log.mark(
                'OOB accuracy',
                `${oobPct !== null && oobPct !== undefined ? oobPct.toFixed(2) : 'null'}%`,
            );
        }
    } else {
        ...
        await log.run(
            ...
            async () => {
                ...
                if (testSize > 0) {
                    ...
                    log.mark(
                        'Test metrics',
                        `acc=${testAccuracyPct !== null && testAccuracyPct !== undefined ? testAccuracyPct.toFixed(2) : 'null'}% kappa=${testKappa !== null && testKappa !== undefined ? testKappa.toFixed(3) : 'null'}`,
                    );
                } else {
                    log.mark(
                        'Test metrics',
                        'testSize=0 (no GT points survived spatial block split)',
                    );
                }
            },
        );
    ...
    // compute RF chain ở AOI lớn → burst memory 6× trên endpoint
    ...
    return {
        ...
        quotas: {
            ...
            inputTestQuota: hasGT
                ? Math.max(
                      minFieldTest,
                      Math.round(
                          (quotasPerClass.reduce((a, b) => a + b, 0) * 0.4) /
                              cfg.CLASS_NAMES.length,
                      ),
                  )
                : 0,
