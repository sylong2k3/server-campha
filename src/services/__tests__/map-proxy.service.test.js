...
const response = (body = '{}', type = 'application/json') => ({
    headers: { get: jest.fn((name) => name === 'content-length' ? String(Buffer.byteLength(body)) : type) },
...
describe('map proxy', () => {
    ...
    test('forces DB WMS layer and ignores client layer selection by contract', async () => {
        ...
        await proxyWms({ geoserver_layer: 'campha:flood' }, { version: '1.3.0', crs: 'EPSG:5899', bbox: '1,2,3,4', width: 256, height: 256, format: 'image/png', transparent: true });
    ...
    test('WFS is fixed to GetFeature and bounded count', async () => {
        ...
        await proxyWfs({ geoserver_layer: 'campha:roads' }, { count: 1000, srsName: 'EPSG:4326', outputFormat: 'application/json' });
        const target = requestGeoserver.mock.calls[0][0];
