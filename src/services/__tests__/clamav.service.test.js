...
const {
    chunkFramer, parseResponse, MalwareDetectedError, ClamAvUnavailableError,
...
describe('ClamAV INSTREAM framing', () => {
    ...
    test('classifies clean, malware and unknown responses', () => {
        ...
        expect(() => parseResponse('stream: Eicar-Signature FOUND\0')).toThrow(MalwareDetectedError);
        expect(() => parseResponse('stream: size limit exceeded ERROR\0')).toThrow(ClamAvUnavailableError);
    });
    test('validates enabled scanner configuration', () => {
        const original = process.env;
        process.env = { ...original, CLAMAV_ENABLED: 'true', CLAMAV_HOST: 'http://bad', CLAMAV_PORT: '3310', CLAMAV_TIMEOUT_MS: '30000' };
        expect(() => require('../clamav.service').getConfig()).toThrow('CLAMAV_HOST');
        process.env.CLAMAV_HOST = '127.0.0.1'; process.env.CLAMAV_PORT = '0';
        expect(() => require('../clamav.service').getConfig()).toThrow('CLAMAV_PORT');
        process.env.CLAMAV_PORT = '3310'; process.env.CLAMAV_TIMEOUT_MS = '1';
        expect(() => require('../clamav.service').getConfig()).toThrow('CLAMAV_TIMEOUT_MS');
        process.env = original;
    });
    test('fails closed when scanner is disabled', async () => {
        await expect(require('../clamav.service').scanStream(Readable.from('x'), null))
            .rejects.toBeInstanceOf(ClamAvUnavailableError);
    });
    test('scans clean stream through clamd TCP protocol', async () => {
        const net = require('net');
        const server = net.createServer((socket) => socket.once('data', () => socket.end('stream: OK\0')));
        await new Promise((resolve) => server.listen(0, '127.0.0.1', resolve));
        try {
            const port = server.address().port;
            await expect(require('../clamav.service').scanStream(
                Readable.from('payload'), { host: '127.0.0.1', port, timeoutMs: 2000 }
