...
const boundedExpiry = (value, fallback) => {
    ...
    if (!Number.isInteger(seconds) || seconds < MIN_PRESIGN_SECONDS || seconds > MAX_PRESIGN_SECONDS) {
        throw new RangeError(`Presigned expiry must be ${MIN_PRESIGN_SECONDS}-${MAX_PRESIGN_SECONDS} seconds`);
    }
    return seconds;
};
...
const safeName = (originalName) => {
    ...
    const base = path.basename(originalName || 'file', path.extname(originalName || ''))
        .normalize('NFKD').replace(/[^a-zA-Z0-9_-]/g, '_').toLowerCase().slice(0, 80) || 'file';
    return `${base}${ext}`;
};

const buildObjectKey = (id, originalName, prefix = '') => {
    if (!/^[a-z0-9-]{1,64}$/.test(String(id))) {throw new TypeError('Object id is invalid');}
    if (prefix && !/^[a-z0-9/_-]{1,80}$/.test(prefix)) {throw new TypeError('Invalid object key prefix');}
    const now = new Date();
...
const uploadStream = async ({ stream, objectKey, mimeType, fileSize, category, quarantine = false }) => {
...
const getPresignedDownloadUrl = async ({ objectKey, category, expireSeconds }) => {
    ...
    const url = await getClient().presignedGetObject(getBucketForCategory(category), objectKey, expire);
    return { url, expiresAt: new Date(Date.now() + expire * 1000) };
};

const statQuarantineObject = (objectKey) => getClient().statObject(getQuarantineBucket(), objectKey);
...
const promoteQuarantineObject = async ({ quarantineKey, objectKey, category, sourceEtag }) => {
    ...
    if (!sourceEtag) {throw new TypeError('sourceEtag is required for promotion');}
    conditions.setMatchETag(sourceEtag);
    await client.copyObject(targetBucket, objectKey, `/${sourceBucket}/${quarantineKey}`, conditions);
    return { bucket: targetBucket, objectKey };
};

const removeQuarantineObject = (objectKey) => getClient().removeObject(getQuarantineBucket(), objectKey);
const removeObject = ({ objectKey, category }) => getClient().removeObject(getBucketForCategory(category), objectKey);
const getObjectStream = ({ objectKey, category }) => getClient().getObject(getBucketForCategory(category), objectKey);
