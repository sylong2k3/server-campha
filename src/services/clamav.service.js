...
const getConfig = () => {
    ...
    if (!Number.isInteger(timeoutMs) || timeoutMs < 1000 || timeoutMs > 120000) {
        throw new Error('CLAMAV_TIMEOUT_MS is invalid');
    }
...
const parseResponse = (response) => {
    ...
    if (/^stream: OK$/i.test(value)) {
        return { clean: true };
    }
    const found = /^stream: (.+) FOUND$/i.exec(value);
    if (found) {
        throw new MalwareDetectedError(found[1]);
    }
    throw new ClamAvUnavailableError(`Unexpected clamd response: ${value.slice(0, 120)}`);
};

const scanStream = (source, config = getConfig()) =>
    new Promise((resolve, reject) => {
        if (!config) {
            return reject(new ClamAvUnavailableError('Malware scanner is disabled'));
        }
        ...
        const timer = setTimeout(
            () => finish(new ClamAvUnavailableError('Malware scan timed out')),
            config.timeoutMs,
        );
        timer.unref?.();
        ...
        socket.on('data', (chunk) => {
            ...
            if (responseBytes > 4096) {
                return done(new ClamAvUnavailableError('clamd response too large'));
            }
            chunks.push(chunk);
            if (chunk.includes(0) || chunk.includes(10)) {
                try {
                    done(null, parseResponse(Buffer.concat(chunks).toString('utf8')));
                } catch (error) {
                    done(error);
                }
            }
