...
async function executeAnalysis(
    ...
    let gtData = {
        counts: { zones: 0, points: 0, byClass: {} },
        zones: { features: [] },
        points: { features: [] },
    };
    ...
    try {
        ...
        try {
            ...
            console.warn(
                `[FOREST-CLS] GT query FAILED (${code}) — fallback không GT: ${gtErr.message}`,
            );
        ...
        const rfResult = await runRfClassification(year, region, region.geometry(), {
            month,
        ...
        if (districtGeoJson.length === 0) {
            throw new Error(
                'Không materialize được đơn vị hành chính Cẩm Phả cho district raster export.',
            );
        }
        ...
        const seededDistrictExports = await log.run(
            ...
            () => repo.insertDistrictExports(snapshot.id, districtSkeletons, cfg.DOWNLOAD_SCALE_M),
        );
        ...
        snapshot = await log.run('Update snapshot → status=completed', () =>
            repo.updateStatus(snapshot.id, 'completed', {
                ...
                oob_accuracy:
                    oobPct !== null && oobPct !== undefined ? Math.round(oobPct * 100) / 100 : null,
                test_accuracy:
                    testAccuracyPct !== null && testAccuracyPct !== undefined
                        ? Math.round(testAccuracyPct * 100) / 100
                        : null,
                test_kappa:
                    testKappa !== null && testKappa !== undefined
                        ? Math.round(testKappa * 1000) / 1000
                        : null,
        ...
        _safe(async () => {
            ...
            if (prior > 0) {
                console.log(
                    `[FOREST-CLS] notification SKIP — đã có ${prior} attempt completed trước (dedup theo year/month)`,
                );
                return;
            }
    ...
    const run = geeQueue
        ...
        .catch(async (error) => {
            if (process.env.GEE_ANALYSIS_CHILD !== 'true') {
                await repo.failActiveRunsForPeriod(year, month, error.message).catch((dbError) => {
                    console.error(
...
const _safe = (fn) => {
    try {
        const r = fn();
        if (r?.catch) {
            r.catch((e) => console.warn('[FOREST-CLS] async helper err:', e.message));
        }
    } catch (e) {
...
async function _notifyForestClassificationCompleted(snapshot, provinceSummary) {
    ...
    const body =
        `Đã hoàn thành phân loại 11 lớp cho kỳ ${period}. ` +
