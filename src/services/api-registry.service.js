...
const fields = async (input, layer) => {
    ...
    if (all.some((x) => !webMap.FIELD.test(x) || !actual.has(x) || blocked.has(x.toLowerCase()))) {
    ...
    if (
        ...
        throw new Api422Error('Trường ghi/tìm kiếm/sắp xếp phải thuộc trường đọc', [
...
const create = async (input, actor) => {
    ...
    if (hasWrite(input.allowedMethods)) {
        tnmt(actor);
    }
...
const update = async (id, input, actor) => {
    ...
    if (
        ['readFields', 'writeFields', 'searchFields', 'allowedMethods', 'defaultSortField'].some(
            (x) => Object.hasOwn(input, x),
        )
    ) {
        tnmt(actor);
    }
...
const issue = async (id, input, actor) => {
    ...
    if (input.scopes.some((x) => x !== 'features:read')) {
        ...
        if (input.scopes.some((x) => needed[x] && !current.allowed_methods.includes(needed[x]))) {
