...
if (process.env.DB_NAME !== 'campha_test') {
    throw new Error('Sprint 5 integration chỉ chạy trên campha_test');
}
...
describe('Sprint 5 CMS PostgreSQL integration', () => {
    beforeAll(async () => {
        ...
        const { rows } = await db.query(
            ...
            [
                `documents/${PREFIX}${suffix}.pdf`,
                `${PREFIX}${suffix}.pdf`,
                userId,
                `documents/${PREFIX}${suffix}.xml`,
                `${PREFIX}${suffix}.xml`,
            ],
        );
    ...
    test('public news SQL hides draft and internal while Vietnamese search works', async () => {
        const published = await repository.createNews(
            {
                title: `${PREFIX}Cẩm Phả`,
                content: 'public',
                visibility: 'public',
                status: 'published',
            },
        ...
        await repository.createNews(
            { title: `${PREFIX}Draft`, content: 'draft', visibility: 'public', status: 'draft' },
            userId,
        );
        await repository.createNews(
            {
        ...
        const result = await repository.listNews(
            { q: `${PREFIX}cam pha`, page: 1, limit: 20 },
            true,
        );
    ...
    test('comments stay pending until moderation and public list only returns approved', async () => {
        const news = await repository.createNews(
            { title: `${PREFIX}Comment`, content: 'x', visibility: 'public', status: 'published' },
            userId,
        );
        const comment = await repository.createComment(news.id, 'Bình luận text', userId);
        expect(
            (await repository.listComments(news.id, { page: 1, limit: 20 }, true)).items,
        ).toHaveLength(0);
    ...
    test('document visibility is enforced in SQL and ready owned XML is accepted', async () => {
        const pub = await repository.createDocument(
            {
                title: `${PREFIX}Public doc`,
                documentCode: `${PREFIX}PUB`,
                issuingAgency: 'UBND',
                visibility: 'public',
                fileObjectId: fileXml.id,
            },
            userId,
        );
    ...
    test('PDF map requires clean PDF and optimistic stale update is rejected', async () => {
        const map = await repository.createPdfMap(
            {
                title: `${PREFIX}Map`,
                scaleLabel: '1:10.000',
                mapYear: 2026,
                ...
                visibility: 'public',
                fileObjectId: filePdf.id,
            },
            userId,
        );
        expect(map).not.toBeNull();
        const updated = await repository.updatePdfMap(
            map.id,
            { title: `${PREFIX}Map updated`, expectedUpdatedAt: map.updated_at },
            userId,
        );
        expect(updated.title).toContain('updated');
        await expect(
            repository.updatePdfMap(
                map.id,
                { title: 'stale', expectedUpdatedAt: map.updated_at },
                userId,
            ),
        ).resolves.toBeNull();
        await expect(
            repository.createPdfMap(
                {
                    title: `${PREFIX}Bad`,
                    scaleLabel: '1:1',
                    mapYear: 2026,
