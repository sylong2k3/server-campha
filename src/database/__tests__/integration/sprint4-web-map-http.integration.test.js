if (process.env.DB_NAME !== 'campha_test') {
    throw new Error(`Sprint 4 HTTP integration chỉ được chạy với DB_NAME=campha_test; received ${process.env.DB_NAME}`);
}
...
jest.mock('../../../middlewares/auth.middleware', () => ({
    optionalAuth: (req, _res, next) => {
        ...
        req.user = role ? {
            id: 1, role, org_id: 1,
            role_permissions: { map: { view: true, view_attributes: true, search_feature: true, view_legend: true, view_3d: true } },
        } : null;
        next();
    },
...
describe('Sprint 4 Web Map HTTP integration', () => {
    beforeAll(async () => {
        ...
        const { rows } = await db.query(`
            ...
            RETURNING *`,
        [`${PREFIX}public_${suffix}`, `${PREFIX}table_public_${suffix}`, `${PREFIX}private_${suffix}`, `${PREFIX}table_private_${suffix}`]);
    ...
    test('all five DB roles obey layer ACL', async () => {
        ...
        for (const role of roles) {
            const response = await request(app).get('/api/v1/web-map/layers').set('x-test-role', role).expect(200);
            const ids = response.body.data.map((layer) => Number(layer.id));
            expect(ids).toContain(Number(publicLayer.id));
            if (role === 'citizen') { expect(ids).toContain(Number(privateLayer.id)); }
            else { expect(ids).not.toContain(Number(privateLayer.id)); }
        }
    });
    ...
    test('legend enforces per-layer ACL and validates search abuse', async () => {
        ...
        await request(app).get(`/api/v1/web-map/layers/${privateLayer.id}/legend`).set('x-test-role', 'citizen').expect(200);
    ...
    test('basemap endpoint is anonymous', async () => {
        ...
        expect(response.body.data).toEqual(expect.arrayContaining([expect.objectContaining({ code: 'osm_standard' })]));
    });
});
