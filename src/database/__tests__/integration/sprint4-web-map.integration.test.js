if (process.env.DB_NAME !== 'campha_test') {
    throw new Error(`Sprint 4 write integration chỉ được chạy với DB_NAME=campha_test; received ${process.env.DB_NAME}`);
}
...
const cleanup = async () => {
    const { rows } = await db.query(`SELECT table_name FROM gis.layers WHERE code LIKE $1`, [`${PREFIX}%`]);
...
describe('Sprint 4 WebGIS integration', () => {
    beforeAll(async () => {
        ...
        for (const visibility of ['public', 'private']) {
            ...
            const { rows: [layer] } = await db.query(`
                ...
                RETURNING *`,
            [`${PREFIX}${visibility}_${suffix}`, `Layer ${visibility}`, table, visibility === 'public']);
            if (visibility === 'public') { publicLayer = layer; } else { privateLayer = layer; }
        }
        await db.query(`INSERT INTO gis.layer_permissions (layer_id, role_code, can_view) VALUES ($1,'citizen',true)`, [privateLayer.id]);
    });
    ...
    test('migration 009 installs search extensions and safe basemap catalog', async () => {
        const { rows } = await db.query(`SELECT extname FROM pg_extension WHERE extname IN ('unaccent','pg_trgm') ORDER BY extname`);
        expect(rows.map((row) => row.extname)).toEqual(['pg_trgm', 'unaccent']);
        const maps = await repository.basemaps();
        expect(maps).toEqual(expect.arrayContaining([expect.objectContaining({ code: 'osm_standard', provider: 'osm' })]));
    ...
    test('anonymous receives public only; ACL role receives private too', async () => {
        ...
        expect(allowed.map((layer) => layer.id)).toEqual(expect.arrayContaining([publicLayer.id, privateLayer.id]));
    ...
    test('Vietnamese unaccent/trigram search and bbox work on dynamic table', async () => {
        const layer = await repository.accessibleLayer(publicLayer.id, null);
        const matches = await repository.searchLayer(layer, 'phuong cam trung', '107.2,20.9,107.5,21.2', 10);
