...
if (process.env.DB_NAME !== 'campha_test') {
    throw new Error(`Write integration chỉ được chạy với DB_NAME=campha_test; received ${process.env.DB_NAME}`);
}
...
const createUser = async ({ email, roleCode = 'citizen', orgCode = 'ubnd_campha', emailVerified = true }) => {
    const passwordHash = await hashPassword(PASSWORD);
    const { rows: [org] } = await db.query(
        'SELECT id FROM auth.organizations WHERE code = $1',
        [orgCode]
    );
    if (!org) {throw new Error(`Organization fixture missing: ${orgCode}`);}
...
describe('Sprint 1 write integration', () => {
    ...
    test('register giữ email unverified; verify token một lần rồi login/rotate/replay thu hồi mọi session', async () => {
        ...
        const registered = await authService.register({
            email, password: PASSWORD, fullName: 'Integration Auth', phone: '',
        }, context);
        ...
        await db.query(
            'UPDATE auth.email_verification_tokens SET token_hash = $2 WHERE id = $1',
            [verification.id, hashToken(rawToken)]
        );
        await authService.verifyEmail({ token: rawToken }, context);
        await expect(authService.verifyEmail({ token: rawToken }, context))
    ...
    test('cross-org list/get/change-role bị giới hạn bởi organization', async () => {
        const actorUser = await createUser({
            email: `${PREFIX}actor@campha.test`, roleCode: 'so_tnmt', orgCode: 'so_tnmt_qn',
        });
        const sameOrg = await createUser({
            email: `${PREFIX}same@campha.test`, orgCode: 'so_tnmt_qn',
        });
        const otherOrg = await createUser({
            email: `${PREFIX}other@campha.test`, orgCode: 'so_xd_qn',
        });
        const actor = {
            id: actorUser.id,
            role: actorUser.role,
        ...
        await expect(userService.getUserById(otherOrg.id, actor))
            .rejects.toMatchObject({ status: 403 });
        await expect(userService.changeUserRole(otherOrg.id, 'so_xd', actor))
            .rejects.toMatchObject({ status: 403 });
    ...
    test('MFA enrollment/login/recovery atomic; TOTP và recovery code không dùng lại', async () => {
        const user = await createUser({
            email: `${PREFIX}mfa@campha.test`, roleCode: 'so_tnmt', orgCode: 'so_tnmt_qn',
        });
        const setupLogin = await authService.login({ email: user.email, password: PASSWORD }, context);
        ...
        await expect(mfaService.confirm({ challengeToken: setupLogin.challengeToken, code: firstCode }, context))
            .rejects.toMatchObject({ status: 401 });

        const recoveryLogin = await authService.login({ email: user.email, password: PASSWORD }, context);
        ...
        const reusedRecoveryLogin = await authService.login({ email: user.email, password: PASSWORD }, context);
        await expect(mfaService.verify({
            challengeToken: reusedRecoveryLogin.challengeToken,
            recoveryCode,
        }, context)).rejects.toMatchObject({ status: 401 });
    });
});
