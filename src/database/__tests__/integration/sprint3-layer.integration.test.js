if (process.env.DB_NAME !== 'campha_test') {
    throw new Error(
        `Sprint 3 write integration chỉ được chạy với DB_NAME=campha_test; received ${process.env.DB_NAME}`,
    );
}
...
const cleanup = async () => {
    await db.query(`DELETE FROM gis.layer_import_jobs WHERE input_payload->>'code' LIKE $1`, [
        `${PREFIX}%`,
    ]);
    await db.query(
        `DELETE FROM gis.layer_cleanup_jobs WHERE layer_id IN (SELECT id FROM gis.layers WHERE code LIKE $1)`,
        [`${PREFIX}%`],
    );
...
describe('Sprint 3 layer write integration', () => {
    beforeAll(async () => {
        ...
        if (!user) {
            throw new Error('so_tnmt role or organization fixture missing');
        }
        actorUser = user;
        const {
            rows: [file],
    ...
    test('migration 007 durable tables and constraints exist', async () => {
        ...
        await expect(
            db.query(`INSERT INTO gis.layers (code, name_vi) VALUES ('BAD-CODE', 'x')`),
    ...
    test('two claims cannot acquire the same import job', async () => {
        ...
        await jobRepository.failImport(
            job.id,
            claimed.filter(Boolean)[0].worker_id,
            'TEST_DONE',
            'fixture cleanup',
        );
    });
    ...
    test('stale import worker cannot complete or fail after lease loss', async () => {
        ...
        await db.query(
            `UPDATE gis.layer_import_jobs SET lease_expires_at = NOW() - INTERVAL '1 second' WHERE id = $1`,
            [job.id],
        );
        await expect(
            jobRepository.completeImport(job.id, 'worker-stale-import', {
                layerId: layer.id,
                featureCount: 1,
                geometryType: 'POINT',
                sourceSrid: 4326,
        ...
        await expect(
            jobRepository.failImport(job.id, 'worker-stale-import', 'STALE', 'stale'),
        ).resolves.toBeNull();
        const {
            rows: [state],
        } = await db.query(`SELECT status, layer_id FROM gis.layer_import_jobs WHERE id = $1`, [
            job.id,
        ]);
    ...
    test('stale cleanup worker cannot update job or layer status', async () => {
        ...
        await expect(
            jobRepository.completeCleanup(job.id, 'worker-stale-cleanup', layer.id),
        ).resolves.toBe(false);
        await expect(jobRepository.failCleanup(job, 'worker-stale-cleanup', 'stale')).resolves.toBe(
            false,
        );
        const {
            rows: [state],
        } = await db.query(`SELECT cleanup_status FROM gis.layers WHERE id = $1`, [layer.id]);
    ...
    test('ACL replacement is atomic and delete enqueues cleanup', async () => {
        ...
        expect(replaced.permissions).toEqual([
            expect.objectContaining({ roleCode: 'citizen', canView: true }),
        ]);
        ...
        const {
            ...
            `SELECT COUNT(*)::int AS value FROM gis.layer_cleanup_jobs WHERE layer_id = $1`,
            [layer.id],
        );
