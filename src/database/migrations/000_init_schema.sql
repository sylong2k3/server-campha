-- ============================================================================
-- Migration 000: INIT SCHEMA — base core (auth) cho server-campha
--
-- Bản base: chỉ còn schema `core` + `auth` (xác thực/phân quyền người dùng).
-- Toàn bộ schema/bảng nghiệp vụ GIS của server-kontum (gis/fire/cms/field)
-- đã bị gỡ — thêm lại bằng migration mới khi phát triển tính năng cụ thể.
--
...
-- ════════════════════════════════════════════════════════════════════════════
--  4. auth.roles — Vai trò (4 vai trò chính của dự án Cẩm Phả)
-- ════════════════════════════════════════════════════════════════════════════
...
VALUES
    ...
    (
        ...
        'Ủy ban Nhân dân thành phố Cẩm Phả — xem dashboard tổng quan, '
        'theo dõi tình hình môi trường toàn tỉnh, xem cảnh báo, '
        'xuất báo cáo tổng hợp, phê duyệt kế hoạch ứng phó.',
        'Cẩm Phả Provincial People''s Committee — view overview dashboard, '
    ...
    (
        ...
        'Sở Nông nghiệp và Môi trường thành phố Cẩm Phả — nhập dữ liệu môi trường thủ công, '
        'quản lý trạm đo/sensor IoT, quản lý vùng giám sát, '
        'xử lý và phản hồi cảnh báo, tạo báo cáo chuyên ngành, upload media.',
        'Cẩm Phả Provincial Department of Agriculture & Environment — manually input environmental data, '
    ...
    (
        ...
        'Người dân',
        'Citizens/Public',
        'Người dân thành phố Cẩm Phả — xem bản đồ môi trường, xem chỉ số chất lượng '
        'không khí/nước, nhận cảnh báo công khai (cháy rừng, ô nhiễm), '
        'xem báo cáo tổng hợp đã công bố.',
        'Cẩm Phả citizens — view environmental maps, air/water quality indices, '
