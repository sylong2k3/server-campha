...
const USER_SELECT = `
    SELECT u.id, u.email, u.password_hash, u.full_name, u.phone, u.avatar_url,
           ...
           u.lockout_level, u.token_version,
...
const create = async ({
    ...
    const { rows } = await db.query(
        `INSERT INTO auth.users (email, password_hash, full_name, phone, avatar_url, role_id, org_id, email_verified, email_verified_at)
         VALUES (
             $1, $2, $3, $4, $5,
             (SELECT id FROM auth.roles WHERE code = $6 AND is_active = true),
             COALESCE($7, (SELECT id FROM auth.organizations WHERE code = 'ubnd_campha' AND $6 = 'citizen')),
...
const incrementLoginAttempts = async (userId, maxAttempts = 5, baseLockMinutes = 15) => {
    const { rows } = await db.query(
        `UPDATE auth.users
         SET login_attempts = CASE
                 WHEN login_attempts + 1 >= $2 THEN 0
                 ELSE login_attempts + 1
             END,
             lockout_level = CASE
                 WHEN login_attempts + 1 >= $2 THEN LEAST(lockout_level + 1, 3)
                 ELSE lockout_level
             END,
             locked_until = CASE
                 WHEN login_attempts + 1 >= $2
                 THEN NOW() + INTERVAL '1 minute' * $3 * POWER(2, LEAST(lockout_level, 3))
                 ELSE locked_until
             END
         WHERE id = $1
...
const findByIdSafe = async (userId) => {
    const { rows } = await db.query(
        `SELECT u.id, u.email, u.full_name, u.phone, u.avatar_url,
                ...
                u.must_change_password, u.token_version, u.lockout_level, u.created_at, u.updated_at,
                (
...
const _buildUserFilter = ({ roleCode, roleCodes, orgId, isActive, q, email }, startIdx = 1) => {
...
const countAll = async ({ roleCode, roleCodes, orgId, isActive, q, email } = {}) => {
    const { where, params } = _buildUserFilter({ roleCode, roleCodes, orgId, isActive, q, email });
