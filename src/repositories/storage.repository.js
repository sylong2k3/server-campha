...
const createQuarantine = async ({ category, bucket, objectKey, quarantineKey, ownerUserId, orgId, originalName, expectedMime, expiresAt }) => {
    const { rows: [row] } = await db.query(
        `INSERT INTO core.file_objects
         ...
         RETURNING *`,
        [category, bucket, objectKey, quarantineKey, ownerUserId, orgId, originalName, expectedMime, expiresAt]
    );
    return row;
};
