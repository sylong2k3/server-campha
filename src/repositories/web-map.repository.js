...
const qid = (value, pattern = IDENTIFIER) => {
    if (!pattern.test(value)) { throw new TypeError('Unsafe database identifier'); }
    return `"${value}"`;
};
...
const safeField = (field) => typeof field === 'string' && FIELD.test(field) && field.toLowerCase() !== 'geom';
const idFieldFor = (layer) => safeField(layer.metadata?.idField)
    ? layer.metadata.idField
    : (layer.metadata?.importType === 'excel' ? 'source_row' : 'source_fid');
const configuredFields = (layer) => {
    const requested = Array.isArray(layer.metadata?.displayFields) ? layer.metadata.displayFields : [];
    const fields = requested.filter(safeField);
    return [...new Set(fields)].slice(0, 50);
};
const searchFields = (layer) => {
    const requested = Array.isArray(layer.metadata?.searchFields) ? layer.metadata.searchFields : [];
