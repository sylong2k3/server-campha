    ...
    try {
        ...
        for (const error of errors) {
            await client.query(
                ...
                [jobId, error.sourceRow || null, error.fieldName || null, error.code, error.message, JSON.stringify(error.details || {})]
            );
        }
        await client.query('COMMIT');
    } catch (error) {
...
const claimCleanup = async (workerId, leaseSeconds = 120) => {
    ...
    try {
        ...
        await client.query("UPDATE gis.layers SET cleanup_status = 'running' WHERE id = $1", [job.layer_id]);
        await client.query('COMMIT');
        return job;
    } catch (error) {
        await client.query('ROLLBACK'); throw error;
