...
const completeEnrollment = async ({
    challengeId, userId, counter, recoveryCodeHashes, tokenVersion, session,
}) => runTransaction(async (client) => {
    if (!await lockEligibleUser(client, userId, tokenVersion)) {return 'user_invalid';}
    if (!await lockChallenge(client, challengeId, userId, 'setup')) {return 'challenge_invalid';}
    ...
    `, [userId, counter]);
    if (rowCount !== 1) {return 'already_enabled';}
    ...
    await client.query('UPDATE auth.mfa_challenges SET used_at = NOW() WHERE id = $1', [challengeId]);
    return 'ok';
});

const completeLogin = async ({
    challengeId, userId, counter, recoveryCodeHash, tokenVersion, session,
}) => runTransaction(async (client) => {
    if (!await lockEligibleUser(client, userId, tokenVersion)) {return 'user_invalid';}
    if (!await lockChallenge(client, challengeId, userId, 'login')) {return 'challenge_invalid';}
    ...
    const factorResult = recoveryCodeHash
        ...
        `, [userId, counter]);
    if (factorResult.rowCount !== 1) {return 'code_invalid';}
