...
const SIGNATURES = Object.freeze({
    ...
    '.webp': [{ offset: 0, bytes: '52494646', secondaryOffset: 8, secondaryAscii: 'WEBP', mime: 'image/webp' }],
    '.pdf': [{ offset: 0, ascii: '%PDF-', mime: 'application/pdf' }],
    '.zip': [{ offset: 0, bytes: '504b0304', mime: 'application/zip' }],
    '.docx': [{ offset: 0, bytes: '504b0304', mime: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' }],
    '.xlsx': [{ offset: 0, bytes: '504b0304', mime: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' }],
    '.pptx': [{ offset: 0, bytes: '504b0304', mime: 'application/vnd.openxmlformats-officedocument.presentationml.presentation' }],
...
const detectFileType = ({ originalName, category, head }) => {
    ...
    const match = rules.find((rule) => {
        const expected = rule.bytes ? Buffer.from(rule.bytes, 'hex') : Buffer.from(rule.ascii, 'ascii');
        ...
        if (rule.secondaryAscii) {
            ...
            return head.subarray(rule.secondaryOffset, rule.secondaryOffset + secondary.length).equals(secondary);
        }
        return true;
    });
