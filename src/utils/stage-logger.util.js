 ...
 *
 * Goal: khi một pipeline GEE bị time-out, log này chỉ ra
 ...
 *   [FOREST-CLS][A][ 0.0s]   ▶ Init Earth Engine
 *   [FOREST-CLS][A][ 0.6s]   ✓ done (612 ms)
 *   [FOREST-CLS][B][ 0.6s]   ▶ Load Cẩm Phả region + units
 *   [FOREST-CLS][B][ 0.7s]   ✓ done (48 ms)
 *
 * Cách dùng:
 *
 *   const { makeStageLogger } = require('../utils/stage-logger.util');
 *   const log = makeStageLogger('FOREST-CLS', { correlationId: snapshot.id });
 *   const region = await log.run('Load Cẩm Phả region', () => Promise.resolve(getCamPhaRegion()));
 ...
 *
 * @param {string} pipelineName - ví dụ 'FOREST-CLS', 'FLOOD-EVENT'
 * @param {object} [opts]
