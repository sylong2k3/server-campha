...
const fail = (code, message) => {
    throw new Api422Error(message, [code]);
};
...
const decodeName = (buffer, utf8) => {
    if (utf8) {
        try {
            ...
            fail('ZIP_FILENAME_ENCODING', 'Tên file ZIP không phải UTF-8 hợp lệ');
        }
    }
...
const inspectShapefileZip = (filePath, overrides = {}) => {
    const limits = {
        ...
        maxEntryBytes:
            Number(overrides.maxEntryMb ?? process.env.LAYER_ZIP_MAX_ENTRY_MB ?? 600) * 1024 * 1024,
        maxExpandedBytes:
            Number(overrides.maxExpandedMb ?? process.env.LAYER_ZIP_MAX_EXPANDED_MB ?? 1000) *
            1024 *
            1024,
        maxCompressionRatio: Number(
            overrides.maxCompressionRatio ?? process.env.LAYER_ZIP_MAX_COMPRESSION_RATIO ?? 100,
        ),
    };
    ...
    for (let index = 0; index < entryCount; index++) {
        ...
        if ((flags & 0x1) !== 0) {
            fail('ZIP_ENCRYPTED', 'ZIP mã hóa không được hỗ trợ');
        }
        if ((flags & 0x8) !== 0) {
            fail('ZIP_DATA_DESCRIPTOR_UNSUPPORTED', 'ZIP data descriptor không được hỗ trợ');
        }
        if (![0, 8].includes(compressionMethod)) {
            fail('ZIP_COMPRESSION_UNSUPPORTED', 'ZIP chỉ hỗ trợ STORE hoặc DEFLATE');
        }
        ...
        if (seen.has(collisionKey)) {
            fail('ZIP_DUPLICATE_ENTRY', 'ZIP có tên file trùng hoặc khác biệt hoa/thường');
        }
        seen.add(collisionKey);
        const unixMode = (externalAttributes >>> 16) & 0xffff;
        const isSymlink = versionMadeBy >>> 8 === 3 && (unixMode & 0xf000) === 0xa000;
        if (isSymlink) {
            fail('ZIP_SYMLINK', 'ZIP không được chứa symbolic link');
        }
        ...
        if (expandedSize > limits.maxEntryBytes) {
            fail('ZIP_ENTRY_TOO_LARGE', 'Một file trong ZIP vượt giới hạn');
        }
        expandedTotal += expandedSize;
        if (expandedTotal > limits.maxExpandedBytes) {
            fail('ZIP_EXPANDED_TOO_LARGE', 'Tổng dữ liệu giải nén vượt giới hạn');
        }
        const ratio = expandedSize === 0 ? 0 : expandedSize / Math.max(1, compressedSize);
        if (ratio > limits.maxCompressionRatio) {
            fail('ZIP_BOMB_RATIO', 'Tỷ lệ nén ZIP vượt giới hạn');
        }
        ...
        if (
            ...
            !buffer
                ...
                .equals(buffer.subarray(nameStart, nameStart + nameLength))
        ) {
            fail('ZIP_LOCAL_ENTRY_MISMATCH', 'Local ZIP entry không khớp central directory');
        }
        if (!isDirectory) {
            entries.push({ name: normalized, compressedSize, expandedSize });
        }
        offset = next;
    }
    ...
    const shapeEntries = entries.filter((entry) =>
        REQUIRED_SHAPEFILE_EXTENSIONS.includes(path.posix.extname(entry.name).toLowerCase()),
    );
    ...
    const complete = [...groups.entries()].filter(([, files]) =>
        REQUIRED_SHAPEFILE_EXTENSIONS.every((ext) => files.has(ext)),
    );
    if (complete.length !== 1 || groups.size !== 1) {
        fail(
            'SHAPEFILE_DATASET_AMBIGUOUS',
            'ZIP phải chứa đúng một bộ .shp/.dbf/.shx/.prj cùng tên',
        );
    }
    ...
    const optionalCpg = entries.find((entry) => {
        ...
        return (
            ext === '.cpg' &&
            entry.name.slice(0, -ext.length).toLocaleLowerCase('en-US') === datasetKey
        );
    });
    return {
        entryCount,
        ...
        files: Object.fromEntries(
            REQUIRED_SHAPEFILE_EXTENSIONS.map((ext) => [ext.slice(1), files.get(ext)]),
        ),
