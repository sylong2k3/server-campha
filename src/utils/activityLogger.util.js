 ...
 */
const logActivity = async (logPrefix, { userId, action, status = 'success', ipAddress, userAgent, metadata }) => {
    try {
        await tokenRepository.logActivity({ userId, action, status, ipAddress, userAgent, metadata });
    } catch (err) {
