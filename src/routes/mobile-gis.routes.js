...
const router=Router(),readLimiter=rateLimit({windowMs:60000,max:180,standardHeaders:true,legacyHeaders:false}),writeLimiter=rateLimit({windowMs:60000,max:30,standardHeaders:true,legacyHeaders:false,keyGenerator:req=>`user:${req.user.id}`});
router.get('/layers/:layerId/tiles/:z/:x/:y',optionalAuth,readLimiter,strict(v.tileParams,'params'),asyncHandler(controller.tile));
router.get('/layers/:layerId/features/:featureId',optionalAuth,readLimiter,strict(v.featureParams,'params'),asyncHandler(controller.feature));
