...
const downloadLimiter = rateLimit({ windowMs: 15 * 60 * 1000, max: 20, standardHeaders: true, legacyHeaders: false, keyGenerator: (req) => `user:${req.user.id}` });
...
publicRouter.get('/images/:id/download-url', verifyToken, enforcePasswordChange, downloadLimiter, strict(v.idParamsSchema, 'params'), strict(v.downloadQuerySchema, 'query'), asyncHandler(controller.download));
...
adminRouter.patch('/images/:id/category', strict(v.idParamsSchema, 'params'), strict(v.categorySchema), asyncHandler(controller.categorize));
adminRouter.delete('/images/:id', strict(v.idParamsSchema, 'params'), strict(v.deleteQuerySchema, 'query'), asyncHandler(controller.remove));
