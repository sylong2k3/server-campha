...
const readSecretFile = (envName) => {
    ...
    if (!file) {throw new Error(`${envName} is required when GEOSERVER_ENABLED=true`);}
...
const validateResourceName = (value, label) => {
    if (!RESOURCE_PATTERN.test(value || '')) {throw new Error(`${label} is invalid`);}
    return value;
};
...
const getConfig = () => {
    ...
    if (!rawUrl) {throw new Error('GEOSERVER_URL is required when GEOSERVER_ENABLED=true');}
    const parsed = new URL(rawUrl);
    if (!['http:', 'https:'].includes(parsed.protocol) || parsed.username || parsed.password
        || parsed.search || parsed.hash) {
        throw new Error('GEOSERVER_URL must be an http(s) server URL without credentials/query/hash');
    }
    const user = process.env.GEOSERVER_USER?.trim();
    if (!user || user.length > 100) {throw new Error('GEOSERVER_USER is invalid');}
    return {
        url: rawUrl.replace(/\/+$/, ''),
        user,
        password: readSecretFile('GEOSERVER_PASSWORD_FILE'),
        workspace: validateResourceName(process.env.GEOSERVER_WORKSPACE || 'campha', 'GEOSERVER_WORKSPACE'),
        namespaceUri: process.env.GEOSERVER_NAMESPACE_URI || 'https://gis.campha.gov.vn/campha',
        datastore: validateResourceName(process.env.GEOSERVER_DATASTORE || 'campha_postgis', 'GEOSERVER_DATASTORE'),
...
const assertGeoserverConfigured = () => {
    ...
    if (!config) {throw new Error('GeoServer is disabled');}
