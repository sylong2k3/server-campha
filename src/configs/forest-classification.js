...
// =====================================================================
// Forest classification config — giữ nguyên mô hình khoa học legacy v5.3
//
// So với v3 (server cũ):
...
// Chỉ số sanity-check vận hành, không tham gia huấn luyện/phân lớp. Giữ giá
// trị hiệu chỉnh legacy mặc định để không âm thầm thay đổi mô hình; production
// Cẩm Phả phải đặt FC_AOI_TOTAL_HA theo ranh giới AOI đã được nghiệm thu.
const PROVINCE_TOTAL_HA = parseFloat(process.env.FC_AOI_TOTAL_HA) || 967417;
...
const EXPECTED_DISTRICT_COUNT = Math.max(
    1,
    parseInt(process.env.FC_EXPECTED_DISTRICT_COUNT, 10) || 1,
);
