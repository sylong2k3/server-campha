...
async function getServiceConnectionStatuses() {
    ...
    const minioStatus = !storageEnabled ? '— Disabled' : minioOk ? '✓ Connected' : '✗ Unavailable';
...
function startServer({ earthEngineStatus, dbStatus, minioStatus, geoserverStatus }) {
    ...
    if (IS_SINGLETON_WORKER) {
        ...
        if (rasterWorker.started) {
            // Do not make a newly queued Forest/Satellite archive wait for the
            // first cron tick after a server restart.
            void rasterIngestWorker.tick();
        }
    ...
    process.on('unhandledRejection', (error) => {
        ...
        console.error(error.name, error.message);
        console.error(error.stack);
        systemLogger.logError('server', `Unhandled promise rejection: ${error.message}`, {
            stack: error.stack,
        });
