...
- **DEPLOYMENT_GATE — WMS tile Authorization header via Mapbox raster source**: Mapbox GL cannot attach a `Bearer` header to tile requests emitted from a raster source URL template. If `/maps/layers/:id/wms` enforces auth, tiles will 401 unless the server issues signed-URL tiles or the client uses `transformRequest` (CORS-dependent). Documented in `mapProxyService.js` header. Requires server-side confirmation of the tile auth model before rollout.
  - **Server-side fix landed 2026-08-13**: `GET /api/v1/maps/layers/:id/tile-ticket?access=view|export` issues a short-lived (~15 min) JWT bound to `layerId`+`access`, after passing the same `requireLayerAccess` check as `/wms`/`/wfs`. Embed it as `&ticket=<value>` in the tile URL template instead of a header — no `transformRequest` needed, works identically on web and native mobile. Public layers (`is_public=true`) are unaffected and still need no ticket. See [tile-ticket.bru](../../api/bruno/Map-Proxy/tile-ticket.bru) and `MOBILE_SERVER_HANDOFF.md`. **Gate stays open until `mapProxyService.js`/`buildRasterTileTemplateUrl` actually fetches the ticket first** — this is only the backend half.
